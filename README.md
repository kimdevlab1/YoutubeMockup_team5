...
- 유튜브 UI 구조 분석 및 화면 구성 요소 분해
  프로젝트 시작 단계에서 YouTube 메인 화면을 분석하여 전체 UI를 다음과 같은 영역으로 분해한다.
  - Header (상단 네비게이션)
  - Filter Chips (카테고리 탭)
  - Sidebar (좌측 메뉴)
  - Video Grid (일반 영상 목록)
...
- HTML을 사용한 기본 구조 작성
  ...
  위와 같이 UI 영역을 명확히 구분하여 레이아웃 구조가 한눈에 보이도록 구성한다.
...
- CSS를 활용한 기본 스타일링 적용
  ...
  styles/
   general.css   // 공통 스타일
   header.css    // Header 영역
   sidebar.css   // Sidebar 영역
   tabs.css      // Filter Chips
   video.css     // Video Grid 및 Shorts
  
- 헤더/썸네일 등 핵심 UI 요소 구현
  실제 YouTube UI를 참고하여 시각적 유사성을 높이도록 한다.
...
- 화면설계서 및 결과 문서 작성
  프로젝트 진행 과정과 결과를 README로 정리한다. 하위 태스크 수행 흐름과 역할 분담이 문서에 드러나도록 작성한다.
